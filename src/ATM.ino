...
void setup()
{
  ...
  tuner.DRIVER.microsteps(MICROSTEPS);
  ...
  matcher.DRIVER.microsteps(MICROSTEPS);
  ...
  tuner.STEPPER.setMaxSpeed(MAXSPEED);
  tuner.STEPPER.setAcceleration(ACCELERATION);
  tuner.STEPPER.setEnablePin(EN_PIN_M1);
  tuner.STEPPER.setPinsInverted(false, false, true);
  tuner.STEPPER.enableOutputs();

  matcher.STEPPER.setMaxSpeed(MAXSPEED);
  matcher.STEPPER.setAcceleration(ACCELERATION);
...
void homeStepper(Stepper stepper)
{
  ...
  stepper.STEPPER.setMaxSpeed(MAXSPEED);
  stepper.STEPPER.setAcceleration(MAXSPEED);
...
uint32_t automaticTM(uint32_t target_frequency)
{
  ...
  if (abs(delta_frequency) > 5000000U)
    resonance_frequency = approximateResonance(target_frequency, resonance_frequency);
